/**
 * Debug runner - starts Java debug sessions for Cucumber tests
 */
...
/**
 * Debugs the selected feature, scenario, or example row
 */
export async function debugSelectedTest(uri: vscode.Uri, lineNumber?: number, exampleLine?: number): Promise<boolean> {
  ...
  try {
    ...
    if (!gluePath) {
      ...
      await debugCucumberTest(workspaceFolder.uri.fsPath, relativePath, userInput, lineNumber, exampleLine);
    } else {
      // If glue path is found, debug directly
      let message = '';
      if (runMode === 'feature') {
        message = `Debugging feature file with glue path "${gluePath}"`;
      } else if (runMode === 'scenario') {
        message = `Debugging scenario at line ${lineNumber} with glue path "${gluePath}"`;
      } else if (runMode === 'example') {
        message = `Debugging example at line ${lineNumber}:${exampleLine} with glue path "${gluePath}"`;
      }

      vscode.window.showInformationMessage(message);
      await debugCucumberTest(workspaceFolder.uri.fsPath, relativePath, gluePath, lineNumber, exampleLine);
    }
  } catch (error: any) {
...
/**
 * Debugs the Cucumber test - IntelliJ-style approach
 */
async function debugCucumberTest(
  projectRoot: string,
  featurePath: string,
  gluePath: string,
  lineNumber?: number,
  ...
  // Determine the name for the debug configuration
  let debugName = 'Cucumber: ';
  ...
  try {
    ...
    if (lineNumber && lineNumber > 0) {
      if (exampleLineNumber && exampleLineNumber > 0) {
        // Debugging an example - use only the example line
        ...
        console.log(`Cucumber debug path (example): ${cucumberPath}`);
      } else {
        ...
        console.log(`Cucumber debug path (scenario): ${cucumberPath}`);
      }
    } else {
      ...
      console.log(`Cucumber debug path (feature): ${cucumberPath}`);
    }
  } catch (error) {
  ...
  await vscode.window.withProgress({
    ...
    // Create debug configuration using io.cucumber.core.cli.Main directly
    ...
    console.log('Starting debug session with config:', JSON.stringify(debugConfig, null, 2));
    ...
    if (!started) {
      vscode.window.showErrorMessage('Failed to start debug session. Make sure you have the Java debugger extension (Language Support for Java by Red Hat or Debugger for Java) installed.');
    }
  });
}
