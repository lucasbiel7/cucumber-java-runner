...
#### Gherkin Rule Keyword Support
- **Added full support for `Rule` keyword**: Feature files can now organize scenarios using Rules
- **Hierarchical Test Explorer**: Rules appear as collapsible nodes with their scenarios nested underneath
- **Run/Debug Rules**: Execute all scenarios within a specific Rule with a single click
- **CodeLens Integration**: Added dedicated Run/Debug buttons for Rule lines
- **Smart Result Tracking**: Test results are correctly tracked for scenarios nested inside Rules

#### Technical Implementation
- **Enhanced Parser**: Added `RuleLineHandler` to recognize and parse `Rule:` lines
- **Updated Type System**: New `RuleInfo` interface to represent rules with their scenarios
- **Recursive Result Processing**: Modified `markChildrenFromResults()` to traverse nested test items (Feature → Rule → Scenario)
- **Improved Test Controller**: Rules are now rendered as test items with proper hierarchy
- **CodeLens Refactoring**: Simplified CodeLens provider to reuse the centralized parser

### 🔧 Code Quality Improvements

#### CodeLens Provider Refactoring
- **Eliminated code duplication**: CodeLens now uses `parseFeatureFile()` from the centralized parser
- **Removed manual parsing**: No more line-by-line iteration with regex checks
- **Cleaner architecture**: CodeLens provider is now 50% smaller and easier to maintain
- **Consistent behavior**: CodeLens and Test Explorer now use the same parsing logic

#### Benefits
- ✅ **Better organization**: Use Rules to group related scenarios logically
- ✅ **Improved readability**: Feature files with Rules are easier to understand
- ✅ **Flexible execution**: Run entire Rules or individual scenarios
- ✅ **Accurate results**: All test results are correctly attributed to their scenarios
- ✅ **Maintainable code**: Single source of truth for feature file parsing
